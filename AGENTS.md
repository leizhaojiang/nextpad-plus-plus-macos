...
- Result lines are written as `"\t" + the line text` — **no NPP "Line NNNN:" prefix** (owner request: in a narrow panel the prefix hid the text). The line number is not displayed but still drives navigation via `_lineInfos`, so double-click still jumps to it. Marking offsets are `1 (tab) + matchStart` in UTF-8 bytes; the prefix-free format is why `src/LexSearchResult.cxx` exists (see "Vendored code").
